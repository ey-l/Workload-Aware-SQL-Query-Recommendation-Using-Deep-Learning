...
def make(config):
    ...
    #d_ff = 768
    # Make model
    model = core.make_model(config.V, config.V, N=config.num_layers, emb_size=config.emb_size, d_model=config.hidden_size, d_ff=config.d_ff, h=config.heads, dropout=config.dropout).to(config.device[0])
